...
class E_waybillupload extends CI_Controller
{
    ...
    function billupload()
    {
        ...
        $Transporter = isset($params[0]['transporter']) ? $params[0]['transporter'] : "";
        ...
        // $store_type       = isset($params[1]['status_count']) ? $params[1]['status_count'] : "";

        if($Transporter != "" && $e_waybill != "") //for coco
        {
...
elseif($Transporter != "" && $e_waybill == "") //for foco warehouse will assign only transporter
{
            $result = $this->Model_connectdb->ebillupload($Transporter,$e_waybill,$apprl_code,$comment);
            if($result)
            {
                ...
                if($data){
                    $array['status_code'] = 200;
                    $array['message'] = "Transporter assigned";
                    echo json_encode($array);
                }
...
elseif($Transporter == "" && $e_waybill != "") //for foco warehouse will assign only transporter
{
            ...
            if($result)
            {
                ...
                if($data){
                    $array['status_code'] = 200;
                    $array['message'] = "Eway bill uploaded";
                    echo json_encode($array);
                }
