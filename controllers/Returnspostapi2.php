...
class Returnspostapi2 extends CI_Controller
{
...
$insertingtime = microtime(true);
            if($qry)
                {
                for($i=0;$i<sizeof($exceldata);$i++)
                    {
                        $fromstrcode = isset($exceldata[$i]['ISSUING STORE CODE']) ? $exceldata[$i]['ISSUING STORE CODE'] : "";
                        $rcvstrname  = isset($exceldata[$i]['RECEIVING STORE CODE']) ? $exceldata[$i]['RECEIVING STORE CODE'] : "";
                ...
                $updatingapprtime = microtime(true);
                if ($updateapprcode) {
                    ...
                    $inserting2ndtabletime = microtime(true); 
                    if($apprvlvarinc > 0)
                    {
            ...
            if ($openreturnsarr != "") {
                ...
                $time[0]['totaltime'] = $endtime - $starttime ;
                $time[0]['excelreadtime'] = $starttime - $excelreadtime;
                $time[0]['validatetime'] = $validatetime - $starttime;
                $time[0]['strformtime'] = $strformtime - $validatetime;
                $time[0]['insertingtime'] = $insertingtime - $strformtime;
                $time[0]['updatingapprtime'] = $updatingapprtime - $insertingtime;
                $time[0]['inserting2ndtabletime'] = $inserting2ndtabletime - $updatingapprtime;
                $time[0]['response'] = $endtime - $inserting2ndtabletime;
